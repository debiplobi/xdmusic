...
import { MediaSkeletonGrid, SongSkeletonList } from "@/components/shared/MediaSkeleton";
import { MediaCard } from "@/components/shared/MediaCard";
import { SongCard } from "@/components/shared/SongCard";
import { Song, Album, Artist, Playlist, TopQueryItem } from "@/lib/types";
...
const Search: React.FC<PropTypes> = ({ searchText }) => {
  ...
  const handleTopQueryClick = (result: TopQueryItem, index: number) => {
    ...
    if (result.type === "song") {
      ...
      const foundIndex = data?.songs.results.findIndex(s => s.id === result.id) ?? -1;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {data.topQuery.results.map((result, index) => (
                    <MediaCard
                      ...
                      imageUrl={result.image?.[result.image.length - 1]?.url || result.image?.[0]?.url}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {data.albums.results.map((album) => (
                    <MediaCard
                      ...
                      imageUrl={album.image?.[album.image.length - 1]?.url || album.image?.[0]?.url}
                      type="album"
                    />
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {data.artists.results.map((artist) => (
                    <MediaCard
                      ...
                      imageUrl={artist.image?.[artist.image.length - 1]?.url || artist.image?.[0]?.url}
                      type="artist"
                    />
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {data.playlists.results.map((playlist) => (
                    <MediaCard
                      ...
                      imageUrl={playlist.image?.[playlist.image.length - 1]?.url || playlist.image?.[0]?.url}
                      type="playlist"
                    />
                  ))}
                </div>
