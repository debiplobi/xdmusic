...
const getHref = (type: MediaCardProps["type"], id: string | number, url?: string) => {
    switch (type) {
        case "album":
            return `/album/${id}`;
        case "artist":
...
export const MediaCard: React.FC<MediaCardProps> = ({
    ...
    const content = (
        <Card className={`h-full ${onClick ? 'cursor-pointer' : ''}`} onClick={onClick}>
            <CardContent className="p-0 h-full flex flex-col">
                ...
                <div className={`p-3 flex-grow ${type === "artist" ? "text-center" : ""}`}>
    ...
    return <Link href={getHref(type, id, url)} className="block h-full transition-transform hover:scale-105">{content}</Link>;
};
